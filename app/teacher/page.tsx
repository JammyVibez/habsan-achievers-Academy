...
export default async function TeacherDashboardPage() {
  ...
  const assignedClasses = [...new Set(teacher.subjects.map((s) => s.classLevel.trim()))]
  const studentsCount =
    assignedClasses.length > 0
      ? await prisma.student.count({ where: { classLevel: { in: assignedClasses } } })
      : homeroom
  ...
  const stats = [
    {
      ...
      description:
        assignedClasses.length > 0
          ? `${assignedClasses.length} assigned class(es)`
          : homeroom
            ? `Homeroom ${homeroom}`
    ...
    },
    {
      ...
      value: String(new Set(teacher.subjects.map((s) => s.subject.name)).size),
      icon: BookOpen,
      description:
        [...new Set(teacher.subjects.map((s) => s.subject.name))].slice(0, 2).join(", ") || "No subjects assigned",
    },
    {
          ...
          <CardContent>
            <div className="space-y-3">
              {(teacher.subjects.length === 0
                ...
                : teacher.subjects.map((s) => ({ subject: s.subject.name, class: s.classLevel || homeroom || "—" }))).map((schedule, index) => (
